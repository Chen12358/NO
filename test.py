...
if __name__ == "__main__":
    ...
    # 画 3 个样本（保持你原来“蓝绿”风格：Ground-truth 和 prediction 不指定 cmap）
    for index in range(3):
        ...
        ax.imshow(x.cpu().squeeze().detach().numpy(), cmap="gray")  # 只对输入用灰度（可改成不带 cmap）
        if index == 0:
            ax.set_title("Input x")
        ax.set_xticks([]); ax.set_yticks([])

        ax = fig.add_subplot(3, 3, index * 3 + 2)
        ax.imshow(y.cpu().squeeze().detach().numpy())  # 默认 colormap（蓝绿）
        if index == 0:
            ax.set_title("Ground-truth y")
        ax.set_xticks([]); ax.set_yticks([])

        ax = fig.add_subplot(3, 3, index * 3 + 3)
        ax.imshow(out_img.cpu().squeeze().detach().numpy())  # 默认 colormap（蓝绿）
        if index == 0:
